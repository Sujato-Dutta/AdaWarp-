...
read -r -a ABLATIONS <<< "${ADAWARP_MVPF_ABLATIONS:-$ABLATIONS_DEFAULT}"

OUTPUT_ROOT="${ADAWARP_OUTPUT_ROOT:-results/adawarp_mvpf_ablation}"
...
PY
} > "$OUTPUT_ROOT/audit/adawarp_mvpf_ablation_config.txt"

python benchmark_adawarp_mvpf_ablation.py \
  --ablations "${ABLATIONS[@]}" \
  --datasets "${DATASETS[@]}" \
  --horizons "${HORIZONS[@]}" \
  --seeds "${SEEDS[@]}" \
...
if [[ -n "${ADAWARP_MVPF_FULL_ROOT:-}" ]]; then
  python aggregate_adawarp_mvpf_ablations.py --root "$OUTPUT_ROOT" --full-root "$ADAWARP_MVPF_FULL_ROOT"
else
  python aggregate_adawarp_mvpf_ablations.py --root "$OUTPUT_ROOT"
fi

echo "[run_adawarp_mvpf_ablation] complete output_root=$OUTPUT_ROOT"
