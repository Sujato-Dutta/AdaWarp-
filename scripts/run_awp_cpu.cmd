...
echo Usage: scripts\run_awp_cpu.cmd [smoke^|short^|full] [dataset ...]
exit /b 2
...
if "%OUTPUT_DIR%"=="" set "OUTPUT_DIR=out\awp_motion_code_cpu"
...
set "DATASETS=PDSetting1 PDSetting2 PronunciationAudio ECGFiveDays FreezerSmallTrain HouseTwenty InsectEPGRegularTrain ItalyPowerDemand Lightning7 MoteStrain PowerCons SonyAIBORobotSurface2"

:run
for %%D in (%DATASETS%) do (
    for %%S in (%SEEDS%) do (
        "%PYTHON_BIN%" -u benchmark_awp_motion_code.py --dataset "%%D" --seed "%%S" --device cpu --output-dir "%OUTPUT_DIR%" %PROFILE_ARGS%
